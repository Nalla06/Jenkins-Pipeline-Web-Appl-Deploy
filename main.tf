terraform {
  ...
  backend "s3" {
    bucket = "jenkins-project-backend-nalla"
...
resource "aws_instance" "managed_nodes" {
  ...
  key_name = "linux-key-pair"  # change with your pem file
...
# we can use user_data to speed up the running of the playbook. (we have update task in playbook, too)
  user_data = <<-EOF
            #! /bin/bash
            dnf update -y
            EOF
...
resource "aws_security_group" "tf-sec-gr" {
  name = "project208-sec-gr-${var.user}-p"
  tags = {
